...
def conv_temp2(inputs, nr_frames, scope=None):
    ...
    inputs = tf.reshape(
        inputs, tf.stack([-1, nr_frames, spatial_x, spatial_y, channels]))

    value = np.zeros((1, 3, 1, 1, channels))
    value[0, 1, 0, 0, :] = 1
    init = tf.constant_initializer(value)

    feats = slim.conv3d(inputs, channels, [3, 1, 1], stride=1,
                        rate=1, padding='SAME', scope=scope, weights_initializer=init)
    feats = tf.reshape(
        feats, [-1, spatial_x, spatial_y, channels])
    return feats
