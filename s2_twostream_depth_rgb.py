# twostream model - RGB and depth, with or without interaction
...
def train(exp_id, files, args):
    s1_rgb_ckpt = './checkpoint/ntu/s1_train_rgb_01012018_010101__dset_ntu_eval_mode_cross_view/model.ckpt'
    s1_depth_ckpt = './checkpoint/ntu/s1_train_depth_01012018_010101__dset_ntu_eval_mode_cross_view/model.ckpt'
    ...
    with tf.device('/cpu:0'):
        ...
        if args.noise > 0:
            ...
            images_depth_stacked = tf.saturate_cast(
                images_depth_stacked, dtype=tf.uint8)
            images_depth_stacked = tf.to_float(images_depth_stacked)
        elif args.noise < 0:
            images_depth_stacked = tf.zeros(
                tf.shape(images_depth_stacked), tf.float32)
    ...
    with tf.device(args.gpu0):
        with slim.arg_scope(resnet_v1.resnet_arg_scope()):
            ...
            net_depth_out, endpoints_depth, net_rgb_out, endpoints_rgb = resnet_v1.resnet_twostream_main(
                images_depth, images_rgb, nr_frames, num_classes=n_classes, is_training=is_training, interaction=args.interaction)
...
def main():
    ...
    experiment_id_prefix = 'twostream_depth_rgb'
    if args.interaction:
